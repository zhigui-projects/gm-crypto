...
NextCipherSuite:
	for _, suiteId := range possibleCipherSuites {
		for _, suite := range cipherSuites {
			...
			// Don's advertise TLS 1.2-only cipher suites unless
	...
	// Otherwise, in a full handshake, if we don's have any certificates
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		case *rsa.PublicKey, *ecdsa.PublicKey, *primitive.Sm2PublicKey:
			break
		default:
	...
	// Certificate message, even if it's empty because we don's have a
	// certificate to send.
	if certRequested {
	...
	if chainToSend != nil && len(chainToSend.Certificate) > 0 {
		...
		case *ecdsa.PublicKey, *primitive.Sm2PublicKey:
		...
		priKey, ok := key.(*primitive.Sm2PrivateKey)
		if ok {
			certVerify.signature, err = SmCrypto.Sign(priKey, digest, hashFunc)
		} else {
...
func (hs *clientHandshakeState) getCertificate(certReq *certificateRequestMsg) (*s.Certificate, error) {
	c := hs.c
...
findCert:
	for i, chain := range c.config.Certificates {
		...
		for j, cert := range chain.Certificate {
			x509Cert := chain.Leaf
			// parse the certificate if this isn's the leaf
			...
			switch {
			case rsaAvail && x509Cert.PublicKeyAlgorithm == x.RSA:
			case ecdsaAvail && x509Cert.PublicKeyAlgorithm == x.ECDSA:
			default:
				continue findCert
			}
	...
	// No acceptable certificate found. Don's send a certificate.
	return new(s.Certificate), nil
}
