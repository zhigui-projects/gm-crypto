...
func (c *Conn) serverHandshake() error {
	...
	if isResume {
		...
		// The client didn's include a session ticket, or it wasn's
...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	if c.config.ClientAuth >= RequestClientCert {
		...
		if len(certMsg.certificates) == 0 {
			// The client didn's actually send a certificate
	...
	if len(c.peerCertificates) > 0 {
		...
		case *ecdsa.PublicKey:
			...
			case SmCrypto.Sm2P256Curve():
				_, err = SmCrypto.Verify(&primitive.Sm2PublicKey{
					X:     key.X,
					Y:     key.Y,
					Curve: key.Curve,
...
func (hs *serverHandshakeState) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	if c.config.ClientAuth >= VerifyClientCertIfGiven && len(certs) > 0 {
		opts := x.VerifyOptions{
			...
			KeyUsages:     []x.ExtKeyUsage{x.ExtKeyUsageClientAuth},
		}
	...
	case *ecdsa.PublicKey, *rsa.PublicKey, *primitive.Sm2PublicKey:
		pub = key
	default:
...
func (hs *serverHandshakeState) setCipherSuite(id uint16, supportedCipherSuites []uint16, version uint16) bool {
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			// Don's select a ciphersuite which we can's
