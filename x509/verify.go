...
func (h HostnameError) Error() string {
	...
	if !hasSANExtension(c) && !validHostname(c.Subject.CommonName) &&
...
func isValid(c *x.Certificate,certType int, currentChain []*x.Certificate, opts *x.VerifyOptions) error {
	...
	vcheckNameConstraints := (certType == intermediateCertificate || certType == rootCertificate) && hasNameConstraints(c)
	if vcheckNameConstraints && commonNameAsHostname(leaf) {
	...
	} else if vcheckNameConstraints && hasSANExtension(leaf) {
...
func Verify(c *x.Certificate, opts x.VerifyOptions) (chains [][]*x.Certificate, err error) {
	...
	if len(keyUsages) == 0 {
		keyUsages = []x.ExtKeyUsage{x.ExtKeyUsageServerAuth}
	}

	// If any key usage is acceptable then we're done.
	for _, usage := range keyUsages {
		if usage == x.ExtKeyUsageAny {
...
func appendToFreshChain(chain []*x.Certificate, cert *x.Certificate) []*x.Certificate {
	n := make([]*x.Certificate, len(chain)+1)
	copy(n, chain)
	n[len(chain)] = cert
	return n
}
...
func buildChains(c *x.Certificate,cache map[*x.Certificate][][]*x.Certificate, currentChain []*x.Certificate, sigChecks *int, opts *x.VerifyOptions) (chains [][]*x.Certificate, err error) {
	...
	considerCandidate := func(certType int, candidate *x.Certificate) {
		...
		*sigChecks++
		...
		case intermediateCertificate:
			if cache == nil {
				cache = make(map[*x.Certificate][][]*x.Certificate)
			}
...
func checkChainForKeyUsage(chain []*x.Certificate, keyUsages []x.ExtKeyUsage) bool {
	usages := make([]x.ExtKeyUsage, len(keyUsages))
...
NextCert:
	for i := len(chain) - 1; i >= 0; i-- {
		...
		const invalidUsage x.ExtKeyUsage = -1
	...
	NextRequestedUsage:
		for i, requestedUsage := range usages {
			...
			for _, usage := range cert.ExtKeyUsage {
				...
				} else if requestedUsage == x.ExtKeyUsageServerAuth &&
					(usage == x.ExtKeyUsageNetscapeServerGatedCrypto ||
						usage == x.ExtKeyUsageMicrosoftServerGatedCrypto) {
